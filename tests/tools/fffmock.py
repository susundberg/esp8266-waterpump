...
class Mock_Generator:
   ...
   def _make_fff_fun( self, node_name, node, fun_par = None ):   
      ...
      fake_params = fake_params.replace("&","")
   ...
   def _make_class_fun( self, class_name, node ):
         ...
         fun_fff_par_names  = ", ".join( [ make_par_name(x[1]) for x in fun_params if make_par_name(x[1]) != "" ] )
         ...
         fun_cls_par_types = ", ".join( [ "%s %s" % (x[0],make_par_name(x[1])) for x in process_params( node ) ] )
...
def process_type( partype ):
    ...
    extra = ""
    if partype.reference:
       extra = "&"
    if partype.pointer:
       extra = "*"
    
    #if partype.full_name() == "THandlerFunction":
       #print "FOO"
       #import ipdb; ipdb.set_trace();
       #print "FOO"
       
    ret = "%s %s%s" % (" ".join(partype.modifiers), partype.full_name(), extra )
    return ret.strip()
